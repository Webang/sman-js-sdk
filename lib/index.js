...
(function (global, factory) {
    ...
    function resolveStack(e) {
      ...
      list.forEach((element, index) => {
        if (index !== 0) {
          ...
          const [protol, filename, lineno, colno] = arg3.replace(/\s/g, '').replace('(', '').replace(')', '').split(':');
          frames.push({
            filename: protol + ':' + filename,
            // 文件名
            colno: +colno,
            // 列数
            function: fun,
  ...
  const sman = {
    init(opt) {
      ...
      initXhr(this);
      initErrorEvent(this);
      initPerformance(this); // SDK 数据上报接口URL

      this.addErrorUrl = opt.url + '/report/error';
      this.addGatewayUrl = opt.url + '/report/gateway';
      this.addPageViewUrl = opt.url + '/report/pageview'; // 以下URL的请求不上报，避免死循环

      this.ajaxWhiteUrl = [this.addErrorUrl, this.addGatewayUrl, this.addPageViewUrl];
    },
